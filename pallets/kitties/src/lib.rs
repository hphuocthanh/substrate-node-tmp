...
pub use super::*;
	...
	pub trait Config: frame_system::Config {
        ...
        type MyRandomness: Randomness<Vec<u8>, Self::BlockNumber>;

        #[pallet::constant]
		type OwnLimit: Get<u32>;
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_kitty(origin: OriginFor<T>, dna: Vec<u8>) -> DispatchResult {
            ...
            // check if the current kitties exceed limit
            ensure!(KittiesOwned::<T>::get(&owner).len() <= T::OwnLimit::get() as usize, Error::<T>::TooManyOwned);

			let gender = Self::gen_gender(&dna)?;
            let (rand_dna, _) = T::MyRandomness::random(&dna);
			let kitty = Kitty::<T> { dna: rand_dna, price: 0, gender, owner: owner.clone(), created_date: Self::now()  };
